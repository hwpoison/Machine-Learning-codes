...
TEXT_REFORCE_DESC = "Por cada entrenamiento, refuerza el aprendizaje repasando nuevamente\n"
...
class InputGrid(Frame):
	...
	def addButton(self, text):
		...
		boton.bind('<ButtonPress-3>', lambda x:self.update_input(boton, mode='m_selection'))
		boton.bind('<ButtonPress-1>', lambda x:self.update_input(boton, mode='normal'))

		return boton
	
	def draw(self, inputs):
		# Dibuja patrón proveniente de la red
		for button, input_ in zip(self.button_grid, inputs[0]):
...
class App(object):
	...
	def init_panel(self):
		...
		Button(control, text=TEXT_TRAIN,command=lambda:self.train()).pack(side=LEFT)
		Button(control, text=TEXT_PREDICT,command=lambda:self.predict()).pack(side=LEFT)

		clear_btn = Button(control, text=TEXT_CLEAR,command=lambda:self.clear())
		CreateToolTip(clear_btn, TEXT_CLEAR_DESC)
		clear_btn.pack(side=LEFT)
		
		forget_btn = Button(control, text=TEXT_FORGET,command=lambda:self.forget())
		CreateToolTip(forget_btn, TEXT_FORGET_DESC)
		forget_btn.pack(side=LEFT)
		
		reinforcement_chk = Checkbutton(control, text=TEXT_REFORCE, variable =self.reinforcement_data)
